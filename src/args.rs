...
pub struct Args {
    ...
    /// Output directory for the generated Bedrock .mcworld file (optional, defaults to Desktop).
...
/// Bedrock Edition is the only output format.
/// `--output-dir` is optional (defaults to Desktop). If provided it must be an existing directory.
...
mod tests {
    ...
    #[test]
    fn test_output_dir_must_exist_if_provided() {
        // Bedrock: if --output-dir is given it must already exist
        let cmd = [
            "arnis",
            "--output-dir",
            "/nonexistent/path",
