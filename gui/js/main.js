...
function initSettings() {
  ...
  if (generationModeSelect) {
    generationModeSelect.addEventListener("change", () => {
      const selectedMode = generationModeSelect.value;
      const mapIframe = document.querySelector('iframe[src="maps.html"]');
      if (mapIframe && mapIframe.contentWindow) {
        mapIframe.contentWindow.postMessage({
    ...
    // Fire it once on load to initialize the state correctly
    setTimeout(() => {
        generationModeSelect.dispatchEvent(new Event("change"));
    }, 1000); // 1s delay to ensure iframe is loaded
  }
...
async function startGeneration() {
  try {
    ...
    var customOsmData = null;
    if (generationMode === "canvas-mode") {
      terrain = true;
      skipOsmObjects = false;
      if (mapFrame && mapFrame.contentWindow && mapFrame.contentWindow.getMockOsmData) {
         customOsmData = mapFrame.contentWindow.getMockOsmData();
      }
    }
