...
$(document).ready(function () {
    ...
    */
    ...
    map.on('draw:created', function (e) {
        ...
        // Assign a feature object to polygon/polyline so we can store OSM tags
        if (e.layerType === 'polygon' || e.layerType === 'polyline') {
            e.layer.feature = e.layer.feature || { type: 'Feature', properties: {} };
            e.layer.feature.properties.osmTags = {};

            if (window.ActiveBrush && brushConfigs[window.ActiveBrush]) {
                // AUTO TAGGING (Smart Brush)
                var tags = brushConfigs[window.ActiveBrush].tags;
                for (var key in tags) { e.layer.feature.properties.osmTags[key] = tags[key]; }
                
                // Keep the color styling from the brush config
                var config = brushConfigs[window.ActiveBrush];
                e.layer.setStyle({ color: config.color, fillColor: config.color });
                e.layer.bindTooltip("Brush: " + window.ActiveBrush, {permanent: false, direction: 'center'}).openTooltip();
                
                // Restart drawing so user can paint multiple instantly
                setTimeout(function() {
                    if (window.ActiveBrush && currentDrawingHandler) {
                        currentDrawingHandler.enable();
                    }
                }, 10);
            } else {
                // MANUAL TAGGING (if no brush selected but user used standard draw tool)
                var exampleTag = e.layerType === 'polygon' ? "building=yes" : "highway=residential";
                var tag = prompt("Enter feature tag for this shape (e.g. " + exampleTag + "):", exampleTag);
                if (tag) {
                    var parts = tag.split('=');
                    var key = parts[0] ? parts[0].trim() : tag.trim();
                    var val = parts[1] ? parts[1].trim() : 'yes';
                    e.layer.feature.properties.osmTags[key] = val;
                    e.layer.bindTooltip(key + "=" + val, {permanent: false, direction: 'center'}).openTooltip();
                }
            }
        }
...
// ========== Smart Brush Palette Logic ==========
window.ActiveBrush = null;
var currentDrawingHandler = null;

var brushConfigs = {
    'forest': { type: 'polygon', color: '#2e8b57', tags: {'natural': 'wood'} },
    'water': { type: 'polygon', color: '#1e90ff', tags: {'natural': 'water'} },
    'building': { type: 'polygon', color: '#dc143c', tags: {'building': 'yes'} },
    'road': { type: 'polyline', color: '#696969', tags: {'highway': 'residential'} }
};

$(document).ready(function() {
    $('.brush-btn').on('click', function(e) {
        e.preventDefault();
        var brushId = $(this).data('brush');

        // Cancel existing handler if running
        if (currentDrawingHandler) {
            currentDrawingHandler.disable();
            currentDrawingHandler = null;
        }

        // Toggle off if already active
        if (window.ActiveBrush === brushId) {
            window.ActiveBrush = null;
        ...
        // Activate new brush
        window.ActiveBrush = brushId;
        $('.brush-btn').removeClass('active');
        $(this).addClass('active');

        var config = brushConfigs[brushId];
        var shapeOptions = {
            color: config.color,
            weight: 3,
            opacity: 0.8,
        ...
        if (config.type === 'polygon') {
            currentDrawingHandler = new L.Draw.Polygon(map, { shapeOptions: shapeOptions });
        } else if (config.type === 'polyline') {
            currentDrawingHandler = new L.Draw.Polyline(map, { shapeOptions: shapeOptions });
        }
    ...
    // Listen for generation mode changes from parent window to show/hide palette
    window.addEventListener('message', function(event) {
        if (event.data && event.data.type === 'setGenerationMode') {
            if (event.data.mode === 'canvas-mode') {
                $('#brush-palette').addClass('active');
            } else {
                $('#brush-palette').removeClass('active');
                if (currentDrawingHandler) {
                    currentDrawingHandler.disable();
                    currentDrawingHandler = null;
                    window.ActiveBrush = null;
...
// Convert drawn features to Mock OSM Data (Overpass JSON format)
window.getMockOsmData = function() {
    var nextNodeId = 1000000;
    var nextWayId = 2000000;
    
    var elements = [];

    drawnItems.eachLayer(function(layer) {
        if (layer instanceof L.Rectangle || layer instanceof L.Marker) {
            // Ignore the selection rectangle and spawn marker
            return;
        }

        var tags = layer.feature && layer.feature.properties && layer.feature.properties.osmTags ? layer.feature.properties.osmTags : { "building": "yes" };
        var nodeIds = [];
        var latLngs = [];
        ...
        if (layer instanceof L.Polygon) {
            nodeIds.push(nodeIds[0]); // Overpass polygons end with the first node
        }
    ...
    return JSON.stringify({
        version: "0.6",
        generator: "Arnis Canvas Prototype",
        elements: elements
    });
};
